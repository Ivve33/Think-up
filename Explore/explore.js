...
async function initExplorer() {
  try {
    const q = query(collection(db, "universities"), where("isVisible", "==", true));
    ...
    gridElement.innerHTML = `<p style="text-align:center; color:red;">Connection Error.</p>`;
  }
}
...
function renderCards(universities) {
  ...
  if (universities.length === 0) {
    gridElement.innerHTML = `<p style="grid-column:1/-1; text-align:center;">No universities found.</p>`;
    return;
  }
    ...
    const card = `
      <a href="university-details.html?id=${uni.id}" class="uni-card">
        <div class="logo-container">
          <img src="${uni.logo}" alt="${uni.name}" class="uni-logo" loading="lazy">
        </div>
        <h3 class="uni-name">${uni.name}</h3>
        <p class="uni-location">📍 ${uni.location}</p>
        <div class="card-action">View Majors →</div>
      </a>
    `;
